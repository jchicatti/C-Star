/*	Tesis de J.D. Chicatti
	13 octubre del 2023
*/
const versionTag = '0.3.0';
const configConstants = require('../configs/configv0.3.0.js');
...
// Build paths
const logsPath = path.join(basePath, config.logsFolder);
const mediaPath = path.join(basePath, config.mediaFolder);
...
client.on('qr', async (qr) => {
    ...
    await qrcode.toFile(config.qrImageName, qr);
    console.log(`QR code generated and saved as ${config.qrImageName}. Scan it with your phone.`);
});
...
const getOpenAIResponse = async (incomingMessage) => {
	try {
		const response = await fetch('https://api.openai.com/v1/chat/completions', {
			...
			body: JSON.stringify({
				messages: [{ role: 'user', content: lang.systemInitContext + incomingMessage }],
...
function saveMessageToContext(chatId, role, content) {
	db.run(`INSERT INTO user_context (chat_id, role, message) VALUES (?, ?, ?)`,
		[chatId, role, content],
		(err) => { if (err) console.error('Failed to save context:', err); }
	);
}
function trimContext(chatId, maxMessages) {
	db.run(`
		...
		WHERE id IN (
			...
			LIMIT -1 OFFSET ?
		)`, [chatId, maxMessages], (err) => {
			if (err) console.error('Failed to trim context:', err);
	});
}
function loadUserContext(chatId, limit, callback) {
	db.all(`
		SELECT role, message FROM user_context
		WHERE chat_id = ?
		ORDER BY timestamp DESC
		LIMIT ?`, [chatId, limit], (err, rows) => {
			if (err) {
				console.error('Error loading context:', err);
				callback([]);
			} else {
				callback(rows.reverse()); // reverse to keep chronological order
			}
	});
}
function saveServiceUsage(startTime, endTime, counts) {
	db.run(`INSERT INTO service_usage (
		start_time, end_time, txtCount, gptCount, imgCount, vidCount,
		wikiCount, dalleCount, menuCount, infoCount, boxCount, pingCount, invalidCount
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
		startTime.toISOString(), endTime.toISOString(),
		counts.txtCount, counts.gptCount, counts.imgCount, counts.vidCount,
		counts.wikiCount, counts.dalleCount, counts.menuCount, counts.infoCount,
		counts.boxCount, counts.pingCount, counts.invalidCount
	], (err) => {
		if (err) console.error('Failed to save service log:', err);
		else console.log('Service usage log saved.');
	});
}
...
client.on('message', async msg => {
	...
	const greetings = [config.cmdDelimiter + 'hello', 'hi', 'hey', 'hello'];
...
rl.on('line', async (input) => {
    switch(input.trim()) {
		...
		case 'ping':
			functionCounts.pingCount++;
			break;
		case config.cmdDelimiter + config.ollamaCmd:
