...
def to_deg(pair): return [pair[0]*180.0/math.pi, pair[1]*180.0/math.pi]
...
def solve_ik_2link(l1,l2,x,y):
    ...
    if c2 < -1.0 or c2 > 1.0:  # fuera de alcance (geométrico)
      return None
    s2 = math.sqrt(max(0.0, 1.0 - c2*c2))
    th2a = math.atan2(s2, c2)     # codo arriba
    th2b = math.atan2(-s2, c2)    # codo abajo
    k1 = l1 + l2*math.cos(th2a); k2 = l2*math.sin(th2a)
    th1a = math.atan2(y, x) - math.atan2(k2, k1)
    k1b = l1 + l2*math.cos(th2b); k2b = l2*math.sin(th2b)
    th1b = math.atan2(y, x) - math.atan2(k2b, k1b)
    return (th1a, th2a), (th1b, th2b)
...
def main():
    ...
    # longitudes inválidas (con cuáles y sus valores)
    bad = []
    if l1 <= 0: bad.append(("l1", l1))
    if l2 <= 0: bad.append(("l2", l2))
    if bad:
        parts = " ".join([f"{k}={v}" for k,v in bad])
        sys.stderr.write(f"EBADPARAMS:LENS {parts}\n"); sys.exit(2)

    # unidades inválidas
    if units not in ("deg","rad"):
        sys.stderr.write(f"EBADPARAMS:UNITS units={units}\n"); sys.exit(2)

    # alcance detallado
    r = math.hypot(x, y); rmax = l1 + l2; rmin = abs(l1 - l2)
    if r > rmax + 1e-9:
        delta = r - rmax
